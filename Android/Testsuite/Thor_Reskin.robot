...
Test Setup          Run Keyword If    '${PREV TEST STATUS}' == 'FAIL'    Open Application wihout unistall and Navigate to dashboard    ${select_Thor_location}
...
############################# Change the temperature unit on EndDevice ######################################
    ${changeUnitValue}    Set Variable    0
    ...
    ...    ${EndDevice_id}
    sleep    ${sleep_5s}
############################# Change the temperature unit on Application ######################################
    Temperature Unit in Fahrenheit
    sleep    ${sleep_10s}
############################ Set Setpoint below 120F From Mobile App Validating Caution Message On Mobile App #########################
...
############################ Set Setpoint above 120F From Mobile App Validating Caution Message On Mobile App    #########################
...
############################ Set Setpoint below 120F From Equipment and Validating it on Equipment itself #########################
...
############################ Set Setpoint above 120F From Equipment and Validating it on Equipment itself #########################
...
############################ Validating Temperature Value and Caution Message On Mobile App ######################
...
########################## View Weekly Usage Report Data From Mobile Application ############################
...
########################## View Monthly Usage Report Data From Mobile Application ############################
...
########################## View Yearly Usage Report Data From Mobile Application ############################
...
############################ Copy Schedule data to another Day #############################
...
############################ Change the temperature and mode from the detail screen ##########################

    Navigate to Detail Page    ${deviceText}
    sleep    ${Sleep_5s}

############################ Changing Mode From the detail page ###################################
    Swipe    994    2058    937    1537    1000
    ${mode_M_DP}    WH Get Mode
    sleep    ${Sleep_5s}
...
############################ Verfiy Setpoint value in Rheem Device ##################################
    ${setpoint_ED}    Read int return type objvalue From Device
    ...    ${whtrsetp}
    ...    ${Device_Mac_Address}
    ...    ${Device_Mac_Address_In_Formate}
...
########################## Resume the Schedule From the Detail page of the Application ######################
...
############################ Verify Setpoint and mode On Equipment ###############################
    ${setpoint_ED}    Read int return type objvalue From Device
    ...    ${whtrsetp}
    ...    ${Device_Mac_Address}
    ...    ${Device_Mac_Address_In_Formate}
...
############################ Update the temperature and mode from Schedule Screen in App #############################
...
############################ Verify Set Point and Mode On Equipment ###########################
    ${setpoint_ED}    Read int return type objvalue From Device
    ...    ${whtrsetp}
    ...    ${Device_Mac_Address}
    ...    ${Device_Mac_Address_In_Formate}
...
############################ Resume the schedule by changing the mode #############################
...
############################ Verify Setpoint and mode On Equipment ###############################
    ...
    ...    ${EndDevice_id}
    ${changeModeValue}    Set Variable    ${mode_ED}
    Convert To String    ${changeModeValue}
...
TC-34:User should be able to set Heat Pump mode from Equipment
    ...
    ############################ Set Heat Pump Mode From Equipment #########################
...
##########################    Set Enable Mode From Equipment    #########################
...
############################# Verfiy Combustion Health Status ###############################
...
############################ Set Vacation Mode From Equipment #########################
...
############################ Set Vacation Mode From Mobile Application #########################
